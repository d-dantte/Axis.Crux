    ...
    public class SemVer
    {
        ...
        public SemVer(string semver)
        {
            ...
            else
            {
                try
                {
                    IsNonStandardSemver = true;
                    var parts = semver.Split('.');
                    Major = uint.Parse(parts[0]);
                    Minor = parts.Length > 1 ? uint.Parse(parts[1]) : 0;
                    Patch = parts.Length > 2 ? uint.Parse(parts[2]) : 0;
                    Pre = parts.Length > 3 ? parts.JoinUsing(".") : null;
                }
                catch(Exception e)
                {
                    throw new Exception($"Invalid SemVer Format: {semver}", e);
                }
            }
        }
        ...
        public bool IsNonStandardSemver { get; private set; }
